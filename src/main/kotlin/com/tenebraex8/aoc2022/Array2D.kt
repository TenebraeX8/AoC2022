...
class Array2D<T>(val n: Int, val m: Int, initializer: (Int)->T) : Iterable<T>{
    ...
    fun lineIndices() = this.innerStructure.indices
    fun colIndices() = this.innerStructure.first().indices
    ...
    fun indicesColumnwise() = iterator {
    ...
    fun right() = indices()
    fun down() = indicesColumnwise()
    fun left() = iterator{
        lineIndices().forEach {line->
            colIndices().reversed().forEach {col->
                yield(ArrayIndex(line, col))
            }
        }
    }
    fun up() = iterator{
        colIndices().reversed().forEach {col->
            lineIndices().reversed().forEach {line->
                yield(ArrayIndex(line, col))
            }
        }
    }
